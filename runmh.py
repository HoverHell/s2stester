...
if __name__ == '__main__':
    ...
    # use something like
    sample = """
    pd(h_message, simplejson.dumps({
      'body': u'message body.',
      'src': u'bot@testard.hell.orts.ru', 'dst': 'hoverhell1@jabber.ru',
      'html': u'message <b>formatted</b>.', 'class': 'message'}))
    from twisted.words.protocols.jabber.jid import JID
    h_presence.subscribe(JID('thehell@exploit.im'), JID('bot@testard.hell.orts.ru'))
    """
    # and see sources for more stuff.
    ...
    shell = IPShellTwisted(
        ...
        user_ns={'__name__': '__example__',
                 'h_presence': presenceHandler,
                 'h_message': msgHandler,
                 'h_vcard': vcardHandler,
                 'h_outq': outqueueHandler,
                 'pd': ProcessData,
                 's2sbase': s2sbase,
